...
// Log that we're attempting to retrieve user info
error_log("Attempting to retrieve user info from session.");

// Check if $_SESSION['uid'] is set
if (!isset($_SESSION['uid'])) {
    error_log("User ID not found in session.");
} else {
    error_log("User ID from session: " . $_SESSION['uid']);
}
...
if ($user) {
    $user_id = $user->id; 
    error_log("User found. User ID: " . $user_id);
} else {
    ...
    case 'add-post':
        ...
        if ($msg == '') {
            ...
            $d = ORM::for_table('sys_accounts')->create();
             $d->system_id = $user_id;  // Set the user_id here
            $d->account = $account;
...
'
    ...
    case 'edit-post':
        ...
        if ($msg == '') {
            ...
            if ($d) {
                ...
                r2(U . 'accounts/ledger', 'e', $_L['Account_Not_Found']);
            }
        } else {
            r2(U . 'accounts/add', 'e', $msg);
        }
    ...
    case 'delete':
        ...
        if ($d) {
            $d->delete();
            r2(U . 'accounts/ledger', 's', $_L['account_delete_successful']);
        }
