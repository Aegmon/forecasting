...
switch ($action) {
    case 'canvas':
        ...
        $net_worth = ORM::for_table('sys_accounts')
                 ->where('system_id', $user_id)
        ->sum('balance');
        if ($net_worth == '') {
            $net_worth = 0;
...
case 'json_income_vs_exp':
    ...
    // Log a message indicating that the data fetching is starting
    error_log("Fetching data for json_income_vs_exp with user_id: " . $user_id);

    // Try fetching the data and log any errors
    try {
        $data = Dashboard::dataLastTwelveMonthsIncExp($user_id);
        
        if (!$data) {
            // Log a warning if no data is returned
            error_log("No data returned for user_id: " . $user_id);
        } else {
            // Log success and the number of data entries returned
            error_log("Data fetched successfully for user_id: " . $user_id . " with " . count($data) . " entries.");
            
            // Log the actual data (use print_r to convert array/object to string)
            error_log("Data: " . print_r($data, true));  // `true` returns the output as a string
        }
    } catch (Exception $e) {
        // Log any exceptions that occur during data retrieval
        error_log("Error fetching data for json_income_vs_exp: " . $e->getMessage());
    }
    ...
    case 'json_d_chart_data':
        ...
        $data = Dashboard::dataIncExpD('dd',$user_id);
