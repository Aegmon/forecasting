...
class Dashboard
{
   public static function dataLastTwelveMonthsIncExp($system_id)
{
    $months = [];
    ...
    foreach ($months as $month) {
        ...
        $inc[] = ORM::for_table('sys_transactions')
            ->where('type', 'Income')
    ...
    public static function dataIncExpD($select,$system_id)
    {
        $inc = [];
        $exp = [];
   ...
   ->where('system_id', $system_id)
    ...
    public static function graphUpdate($user, $config)
    {
        ...
        $u_a_m = base64_decode(
            'PGRpdiBjbGFzcz0iYWxlcnQgYWxlcnQtaW5mbyBmYWRlIGluIj48YnV0dG9uIGNsYXNzPSJjbG9zZSIgZGF0YS1kaXNtaXNzPSJhbGVydCIgaWQ9ImdfdG9wIj7DlzwvYnV0dG9uPkFuIFVwZGF0ZSBpcyBhdmFpbGFibGUuIEdvIHRvIFNldHRpbmdzIC0+IEFib3V0IHRvIFVwZGF0ZSBMYXRlc3QgVmVyc2lvbi48L2Rpdj4='
        );
        ...
        if ($ib_u_t < $ib_now) {
            ...
            try {
                $raw = ib_http_request(
                    'https://dashboard.cloudonex.com/envato/jsonapi/version_check/',
                    'POST',
                    $arr
                );
            } catch (Exception $e) {
